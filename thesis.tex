...
\begin{itemize}
    \item node-red-contrib-rssi (Input: 0, Output: 1): L'indicatore di potenza del segnale ricevuto (RSSI) è una misura stimata di quanto bene un dispositivo può ricevere segnali dal punto di accesso a cui è collegato.
    \item node-red-contrib-bssid (Input: 0, Output: 1):  BSSID identifica l'insieme di servizi di base del dispositivo. Il più delle volte è associato all'indirizzo MAC del punto di accesso a cui è collegato.
    \item node-red-contrib-essid (Input: 0, Output: 1): Nome del punto di accesso alla rete a cui è collegato il dispositivo.
    \item node-red-contrib-localip: L'indirizzo Ip nella rete locale del dispositivo.
    \item node-red-contrib-publicip (Input: 0, Output: 1): L'indirizzo Ip pubblico su Internet del dispositivo.
    \item node-red-contrib-threshold (Input: 0, Output: 1): Soglia utile al calcolo di un EarthQuake Early Warning.
    \item node-red-contrib-alive (Input: 0, Output: 1): Segnale che indica che il dispositivo è attivo al momento. 
    \item node-red-contrib-timesync (Input: 0, Output: 1): Valore utile alla sincronizzazione temporale del dispositivo.
    \item node-red-contrib-quake (Input: 0, Output: 1): Segnalazione di una vibrazione del dispositivo. 
    \item node-red-contrib-disconnect (Input: 0, Output: 1): Segnale inviato quando un dispositivo viene disconnesso.
    \item node-red-contrib-reboot (Input: 1, Output: 1): Segnala il riavvio del dispositivo. 
    \item node-red-contrib-temperature (Input: 0, Output: 1): Valore che indica la temperatura del dispositivo in $^{\circ}$C.
    \item node-red-contrib-probespeed (Input: 0, Output: 1): ?
    \item node-red-contrib-sigma (Input: 0, Output: 1): ?
    \item node-red-contrib-stream (Input: 0, Output: 1): ?
...
\begin{itemize}
    \item node-red-contrib-get-seismometer-info 
    \item node-red-contrib-list-earthquakes 
    \item node-red-contrib-get-all-seismometers-in-group
    \item node-red-contrib-get-weekly-ranking-for-all-devices-in-group
    \item node-red-contrib-general-statistics-for-groups
    \item node-red-contrib-weekly-statistics-for-groups
    \item node-red-contrib-get-activity-intervals-for-seismometers
    \item node-red-contrib-get-statistics-for-active-seismometers-in-the-latest-minutes
    \item node-red-contrib-get-all-perceptible-earthquakes
\end{itemize}

\subsection{COMMON FOLDER?}
...
Come si nota dalle implementazioni tecniche apportate descritte nel capitolo 3 e dai test svolti, l'interfaccia risulta usabile e richiede la minima configurazione dell'utente per programmare anche azioni complesse senza avere una conoscenza tecnica né di programmazione, né di protocolli di rete.
L'applicativo è stabile, è stato testato più volte con gli utenti, ma soprattutto internamente nel team di sviluppo.
Come abbiamo visto nel capitolo 4 è anche sicuro e i dati della rete di sismometri SeismoCloud e le informazioni personali degli utenti rimangono protetti.
Continueremo ad ascoltare gli utenti di SeismoCloud ricevendo feedback utili e proposte di servizi cercando di far rimanere il sistema comprensibile ed usabile.
\textbf{POI?}
...
Come è stato introdotto nel capitolo 2.1, una funzionalità interessante sarebbe l'integrazione con dispositivi per una smart-home. Anche qui dato l'alto contributo della community esistono moduli pronti all'uso (necessaria configurazione). Ad esempio integrazione con Alexa (\href{https://flows.nodered.org/node/node-red-contrib-alexa-smart-home}{https://flows.nodered.org/node/node-red-contrib-alexa-smart-home}) e Google Home\\ (\href{https://flows.nodered.org/node/node-red-contrib-googlehome}{https://flows.nodered.org/node/node-red-contrib-googlehome}).
