...
Viene utilizzato questo protocollo perché è stato progettato per i dispositivi IoT, dato che questi ultimi hanno, solitamente, una batteria con piccola capienza ed in generale risorse limitate. Rispetto al precedente protocollo HTTP, esso invia messaggi ad alta affidabilità, ha una bassa latenza ed un basso overhead. I client si iscrivono ad un \texttt{topic}, ossia un identificatore che si riferisce ad un dato ben preciso (ad esempio \texttt{sensor/temperature}).
...
\end{itemize}
Un esempio di risorsa in stile REST è: \texttt{example.domain/sensor/sensor-id/connect}, dove \texttt{sensor-id} è l'identificatore unico per un sensore.
...
\end{wrapfigure}
Come si può notare dalla Figura \ref{fig:RSS} (a lato) la \textbf{Rete Sismica SeismoCloud} si estende su tutto il territorio nazionale. Si contano circa 100 sismometri attivi.\\
