...
Questo però non deve contrastare le potenzialità del sistema, gli utenti devono avere \textbf{completa libertà nelle scelte} da loro ideate, gli utenti esperti devono comunque riuscire a programmare delle azioni complesse.

Oltre all'esperienza utente viene esaminata la sicurezza garantendo che: i dati che il sistema produce o di cui fa uso non vengano corrotti dall'esterno o utilizzati in maniera impropria; la privacy degli utenti che utilizzano la piattaforma viene garantita.

La tesi è suddivisa in quattro macrocapitoli: la \textbf{progettazione} del sistema e le scelte effettuate in fase di disegno del progetto; l'\textbf{implementazione} delle funzionalità inserite all'interno del sistema volta all'astrazione e alla semplificazione del processo di automatizzazione dei compiti; uno \textbf{studio della sicurezza} del sistema finalizzata alla ricerca ed alla risoluzione, \textit{ove possibile}, di vulnerabilità; una \textbf{fase di test} per assicurare l'esito positivo del lavoro e/oppure sottolineare eventuali criticità ancora presenti.
...
Se l'utente non inserisce un sismometro valido, quindi molto probabilmente ha cambiato i valori tramite console, viene stampato nei log un segnale di warning.
...
I dati che vengono inviati e ricevuti tramite il protocollo MQTT sono scambiati senza nessun protocollo di cifratura. MQTT prende i dati, li incapsula in un pacchetto TCP/IP e lo invia/riceve.
...
TLS (Transport Security Layer) è un protocollo che cifra i dati e
li invia in maniera sicura, lo stesso che viene utilizzato per cifrar ei dati da HTTP in HTTPS.
...
Questa scelta non è casuale, infatti tanti utenti non esperti servono a verificare che il sistema è adatto a persone che non hanno competenze informatiche elevate, mentre la valutazione dell'esperto serve a assicurare che le buone paratiche di usabilità ed esperienza utente vengano rispettate.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\textwidth]{relazione-tirocinio/Chapter-5/bssid-help.jpg}
...
Viene definita ``base" perché non sono stati sviluppati tanit nodi che consentono azioni che potrebbero essere utili per un utente.
