...
\begin{itemize}
    \item node-red-contrib-rssi
    \item node-red-contrib-bssid
    \item node-red-contrib-localip
    \item node-red-contrib-publicip
    \item node-red-contrib-threshold
    \item node-red-contrib-alive
    \item node-red-contrib-timesync
    \item node-red-contrib-quake
    \item node-red-contrib-disconnect
    \item node-red-contrib-reboot
    \item node-red-contrib-temperature
    \item node-red-contrib-probespeed
    \item node-red-contrib-sigma
    \item node-red-contrib-stream
    \item node-red-contrib-streamdata
    \item node-red-contrib-essid
\end{itemize}
...
Oltre ai vari soliti dispositivi smart (luci, elettrodomestici, TV...) l'utente ha completa libertà (privilegiando l'usabilità e la sicurezza) di programmare da sé programmi che si interfacciano con dispositivi hardware creati personalmente. Ad esempio, programmare un GPIO (General Purpose Input/Output) ad emettere un segnale quando avviene una certa condizione. Quest'ultimo poi potrà essere utilizzato per interfacciarsi con dispositivi che hanno bisogno di essere spenti o accesi in determinate condizioni di sicurezza (generatore elettrico, valvola del gas), dato che il GPIO ha solo due stati.
...
L'odierna implementazione istanzia un client MQTT ogni volta che viene creato un nodo, e viene distrutto quando il nodo viene cancellato dal workspace.
In generale data la brevità dei messaggi trasferiti e l'implementazione orientata all'efficienza del protocollo MQTT, l'applicativo non ne risente molto; ma con il crescere dei gruppi, degli utenti e quindi dei dispositivi coinvolti si potrebbero aver problemi di scalabilità, per questo un unico client potrebbe aiutare l'efficienza del sistema.
